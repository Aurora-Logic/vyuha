...
| G-11 | **Two screens crash into the error boundary instead of showing their error state, and twelve more could.** Found by driving the guided tour across all seventy-one navigable screens: `/collections` and `/masters/portal-links` render "This screen stopped responding", so no `screen.header` appears and the page guide has nothing to point at. The throws are `board.collector.name` (`collections-page.tsx:167`, where the guard reads `=== null` and the value was `undefined`) and `(keys.data ?? []).filter is not a function` (`portal-links-page.tsx:105`). | Nothing - all seventy-one work against conforming responses | **Not fixed: these are other slices' modules and mid-flight.** Being precise about what is and is not proven - the payloads that crashed them were **mine**, probe stubs with the wrong shape, and both contracts are satisfied by a conforming server. What is *not* an artefact is why they crashed rather than degraded: of 66 files calling `apiRequest<T>`, **12 never parse the response** - `use-collections` (10 calls), `use-returns` (8), `use-pricing` (8), `use-mfa` (8), `use-duplicates` (5), `use-portal` (3), `use-lifecycle` (3), `use-lifecycle-analytics` (2), and four component files with one each. The other 54 use `parseOrThrow` and answer shape drift with the error state the screen already has. `use-employees.ts` states the reasoning: "the endpoint is built and deployed separately from this screen", and the failure mode of an unvalidated response is "a crash three components deep in a cell renderer". **Recommend parsing all twelve**; the two-character fix on the collections guard (`== null`) removes one symptom and leaves the class. Sixty-seven of seventy-one screens started their page guide; the other two failures were probe timing, confirmed by re-running them 2/2. |
