...
| REQ-V-12 | **Attachments on a task** — a drawing, a signed challan, a photograph of what arrived damaged. PDF, Word, Excel, PowerPoint or an image, up to 3 MB, through the same platform file pipeline deals use: images are re-encoded, everything else is admitted by its bytes and never by its filename. Read by whoever may read the task; attached by whoever may manage it. The register shows a count, the sheet shows the names. (Owner, 31 Aug 2026.) |
| REQ-V-13 | **The card shows everything, and anything can be hidden.** Priority, due date, assignee, linked record, customer, supplier, items and attachments all appear by default; a Fields control turns any of them off, for the board card and the register row together, remembered per person per device. The title, the status and presence are never hidden — the first two are what a row is, and the third is somebody else being in the record right now. (Owner, 31 Aug 2026.) |
| REQ-V-14 | **Working now.** The task dashboard shows who has a task open at this moment and which task it is, as a live block — the presence roster read whole rather than one record at a time. Yourself included, unlike the avatar on a card: a list of who is working must agree with what a colleague sees of the same moment. Nothing polls; it arrives on the live stream. The dashboard itself is a grid of blocks in the shape Notion uses (owner, 31 Aug 2026), one level deep. (Owner, 31 Aug 2026.) |
