...
describe('a task names stock items (REQ-V-10)', () => {
  it('keeps them in the order they were given', async () => {
    const response = await makeTask({ itemIds: [couplerId, cableId] });
    expect(response.status).toBe(201);
  ...
  it('drops a repeat rather than refusing it', async () => {
    ...
    const response = await makeTask({ itemIds: [cableId, cableId] });
    expect(response.status).toBe(201);
    expect(response.body.items).toHaveLength(1);
  });

  it('replaces the whole list on update, and an empty array clears it', async () => {
    const created = await makeTask({ itemIds: [cableId, couplerId] });
    const id = created.body.id;

    const replaced = await harness.patch<TaskView>(`/tasks/${id}`, { token, body: { itemIds: [couplerId] } });
    expect(replaced.body.items.map((item) => item.itemId)).toEqual([couplerId]);

    const cleared = await harness.patch<TaskView>(`/tasks/${id}`, { token, body: { itemIds: [] } });
  ...
  it('lets an item be added back after it was removed', async () => {
    ...
    const created = await makeTask({ itemIds: [cableId] });
    const id = created.body.id;
    await harness.patch<TaskView>(`/tasks/${id}`, { token, body: { itemIds: [] } });
    const again = await harness.patch<TaskView>(`/tasks/${id}`, { token, body: { itemIds: [cableId] } });
  ...
  it('refuses an item that does not exist, naming which one', async () => {
    ...
    const response = await harness.post<ErrorBody>('/tasks', {
      token,
      body: { title: 'Ghost item', itemIds: [cableId, ghost] },
    });
    expect(response.status).toBe(400);
  ...
  it('refuses more items than a task is meant to carry', async () => {
    ...
    const response = await harness.post<ErrorBody>('/tasks', {
      token,
      body: { title: 'A picking list in disguise', itemIds: tooMany },
    });
    expect(response.status).toBe(400);
  });
});
...
describe('the register filters by them', () => {
  it('finds every task about one customer, one supplier or one item', async () => {
    ...
    await makeTask({ title: `${marker} item`, itemIds: [couplerId] });
  ...
  it('returns a task once however many items it names', async () => {
    ...
    const created = await makeTask({ title: 'Two items, one row', itemIds: [cableId, couplerId] });
