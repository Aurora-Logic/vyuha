...
export const DASHBOARD_KPIS: Record<DashboardKpiMetric, KpiMetricSpec> = {
  'receivables-interest': {
    metric: 'receivables-interest',
    label: 'Receivables interest lost',
    reportKey: 'party-interest-cost',
    params: {},
    compute: (rows, meta) => ({
      value: formatMoney(stated(meta, 'interestLoss', sumColumn(rows, 'interestLoss'))),
      note: 'Interest on overdue customer balances',
    }),
    drillQuery: { report: 'party-interest-cost' },
    tone: 'warning',
  },
  'stock-interest': {
    metric: 'stock-interest',
    label: 'Stock interest cost',
    reportKey: 'stock-interest-cost',
    params: {},
    compute: (rows, meta) => ({
      value: formatMoney(stated(meta, 'interest', sumColumn(rows, 'interest'))),
      note: 'Funding stock past vendor credit days',
    }),
    drillQuery: { report: 'stock-interest-cost' },
    tone: 'warning',
  },
  'invoiced-period': {
