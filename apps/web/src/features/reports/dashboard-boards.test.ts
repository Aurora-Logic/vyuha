...
describe('the KPI tiles', () => {
  ...
  it('leads sales with the invoiced figure and finance with the two interest leaks (owner: essential)', () => {
    expect(SALES_PRESET.tiles[0]?.metric).toBe('invoiced-period');
    expect(FINANCE_PRESET.tiles[0]?.metric).toBe('receivables-interest');
    expect(FINANCE_PRESET.tiles[1]?.metric).toBe('stock-interest');
    expect(FINANCE_PRESET.tiles[2]?.metric).toBe('receivables-exposure');
