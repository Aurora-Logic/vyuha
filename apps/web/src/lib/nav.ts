...
const PROD_ALLOWED_REPORT_PATHS = new Set([
  '/reports',
  '/reports/receivables',
  '/reports/sales-analysis',
]);

const ALL_REPORT_GROUPS: NavGroup[] = [
  {
    label: 'General',
    items: [
      {
...
const REPORT_GROUPS: NavGroup[] = import.meta.env.DEV
  ? ALL_REPORT_GROUPS
  : ALL_REPORT_GROUPS
      .map((group) => ({
        ...group,
        items: group.items.filter((item) => PROD_ALLOWED_REPORT_PATHS.has(item.to.split('?')[0])),
      }))
      .filter((group) => group.items.length > 0);
